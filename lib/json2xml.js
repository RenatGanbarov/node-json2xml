...
/*
 *  Modifications (Renat Ganbarov <renat.ganbarov@gmail.com>):
 *  added ability to add xml attributes for any node, ability to add string value for internal node
 */
    ...
    if (!!json.length && typeof json !== 'string') { //Array
        ...
        Object.keys(json).forEach(function (key) {
            if (key !== opts.attributes_key) {
                ...
                if (opts.attributes_key && json[key][opts.attributes_key]) {
                    Object.keys(json[key][opts.attributes_key]).forEach(function (k) {
                        attributes += util.format(' %s="%s"', k, json[key][opts.attributes_key][k]);
                    });
                }
                
                if( opts.string_value_key && node[opts.string_value_key]){
                    var string_value = node[opts.string_value_key];
                } else {
                ...
                if (inner) {
                    ...
                    result += util.format("<%s%s>%s</%s>", key, attributes, string_value, key);
                } else  {
